...
from core.check_indent import check_page_indentation # ปรับใช้แบบ page-based ให้จบในตัว
...
def run_all_checks(pdf_path: str) -> List[Issue]:
    ...
    # 1. ตรวจสอบขนาดกระดาษ (ตรวจครั้งเดียวทั้งไฟล์)
    paper_issues = check_paper_size(doc)
    if paper_issues: # ถ้าอยากบังคับให้หยุดถ้ากระดาษผิด ให้ uncomment ENFORCE_A4_SIZE
    ...
    for i, page in enumerate(tqdm(doc, desc="validate", unit="page", total=len(doc)), 1):
        ...
        for block in text_data.get("blocks", []):
            ...
            for line in block.get("lines", []): all_lines.append(line)
        all_lines.sort(key=lambda l: l["bbox"][1])

        # 3. ตรวจ Indent (เรียกใช้แบบยกหน้า แต่ส่ง State เข้าไปจัดการต่อ)
        if checks.get("check_indentation") and is_main_content:
            issues.extend(check_page_indentation(
                global_state, page, i, m_left, rules, 
        ...
        for line in all_lines:
            ...
            if l_bbox.intersects(page_num_rect): continue
            if l_bbox.y1 < m_top or l_bbox.y0 > (h - m_bottom): continue
            if is_inside_visual(line["bbox"], visual_rects): continue
            ...
            if checks.get("check_section_seq") and is_main_content:
                ...
                if line_type in ["section", "sub_section", "sub_sub_section"]:
                    ...
                    if seq_error:
                        issues.append(Issue(page=i, code="SECTION_SEQ_ERROR", message=seq_error, bbox=line["bbox"]))

            # --- ตรวจ Font ---
            if checks.get("check_font"):
                issues.extend(check_font(i, spans, font_cfg))

            # 🚨 อัปเดต State (สำคัญ: ต้องอัปเดตบรรทัดต่อบรรทัดเพื่อให้หน้าถัดไปทำงานต่อได้)
            global_state.update_prev_text(line_text)
            global_state.prev_line_x1 = line["bbox"][2]

        # 5. ตรวจ Margin (หลังจากกรองบรรทัดขยะออกหมดแล้ว)
